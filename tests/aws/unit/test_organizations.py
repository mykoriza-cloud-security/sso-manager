...
import boto3
import pytest
from aws.app.lib.aws_organizations import AwsOrganizations
...
def test_list_active_aws_accounts_include_all_organiational_units(
    ...
    active_aws_accounts_via_class = list(
        itertools.chain(*aws_organizations_map.values())
    )
...
def test_list_active_aws_accounts_include_specific_organiational_unit(
    ...
    # Act
    organizational_units_via_boto3 = (
        organizations_client.list_organizational_units_for_parent(ParentId=root_ou_id)[
            "OrganizationalUnits"
        ]
    )
    target_child_ou_id = next(
        (
            obj["Id"]
            for obj in organizational_units_via_boto3
            if obj["Name"] == include_ou_name
        )
    )
    aws_organizations_map = py_aws_organizations.describe_aws_organizational_unit(
        parent_ou_id=target_child_ou_id
    )

    active_aws_accounts_via_class = list(
        itertools.chain(*aws_organizations_map.values())
    )
    expected_aws_accounts_via_local_org_map = list(
        next(
            (
                obj["children"]
                for obj in organization_map
                if obj["name"] == include_ou_name
            )
        )
    )
    ...
    assert len(active_aws_accounts_via_class) == len(
        expected_aws_accounts_via_local_org_map
    )
...
def test_list_active_aws_accounts_exclude_suspended_organizational_unit(
    ...
    suspended_ou_id = next(
        (
            obj["Id"]
            for obj in organizational_units_via_boto3
            if obj["Name"] == ignore_ou_name
        )
    )
    ...
    aws_organizations_map = py_aws_organizations.describe_aws_organizational_unit(
        ou_ignore_list=[suspended_ou_id]
    )
