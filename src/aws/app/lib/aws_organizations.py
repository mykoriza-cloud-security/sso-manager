...
class AwsOrganizations:
    ...
    def describe_aws_organizational_unit(
        self, parent_ou_id: str = "", ou_ignore_list: list = []
    ):
        # Get List of all OUs
        ou_list = []
        ...
        for ou_id in ou_list:
            ...
            aws_accounts_flattened_list = list(
                itertools.chain.from_iterable(
                    (page["Accounts"] for page in accounts_iterator)
                )
            )
